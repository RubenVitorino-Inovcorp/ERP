...
const breadcrumbs = computed(() => [
    ...
    { title: `Editar Encomenda #${props.order.number}`, href: '#' },
])

const form = useForm({
    entity_id: String(props.order.entity_id),
...
function submit() {
    const args = isSupplier.value
        ? { encomendas_fornecedore: props.order.id }
        : { encomenda: props.order.id }

    form.put((routes.value.update as any).url(args))
}
</script>

<template>
    <Head :title="isSupplier ? 'Editar Encomenda Fornecedor' : 'Editar Encomenda'" />
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-semibold tracking-tight text-foreground">
                        {{ isSupplier ? 'Editar Encomenda Fornecedor #' + order.number : 'Editar Encomenda #' + order.number }}
                    </h1>
                    <p class="text-sm text-muted-foreground">
                        {{ isSupplier ? 'Altere as informações da encomenda a fornecedor ou as linhas de artigos.' : 'Altere as informações da encomenda ou as linhas de artigos.' }}
                    </p>
                </div>
