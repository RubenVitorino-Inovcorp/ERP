...
import { index as bankAccountsIndex, update as bankAccountsUpdate } from '@/routes/contas-bancarias';
...
const form = useForm({
    ...
    swift: props.bankAccount.swift || '',
    status: props.bankAccount.status,
});

const submit = () => {
    form.put(bankAccountsUpdate(props.bankAccount.id).url, {
        onSuccess: () => {
