...
const breadcrumbs = [
    ...
    { title: 'Nova Ação', href: '/agenda/acoes-de-eventos/criar' },
];
...
function submit() {
    form.post(route('calendar-actions.store'));
}
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4 md:p-6 max-w-2xl mx-auto w-full">
            <div>
                <h1 class="text-2xl font-semibold tracking-tight text-foreground">Nova Ação de Evento</h1>
                <p class="text-sm text-muted-foreground">Adicione uma nova ação de evento.</p>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-6 rounded-xl border bg-white p-6 shadow-sm dark:bg-white/[0.02]">
                ...
                <div class="flex items-center justify-end gap-3 border-t pt-6">
                    ...
                    <Button type="submit" :disabled="form.processing" class="gap-1.5">
                        ...
                        <span>Criar Ação de Evento</span>
                    </Button>
                </div>
            </form>
        </div>
