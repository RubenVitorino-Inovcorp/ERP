...
const breadcrumbs = [
    ...
    { title: 'Editar Tipo de Evento', href: `/agenda/tipos-de-eventos/${props.calendarType.id}/editar` },
];

const form = useForm({
    name: props.calendarType.name,
});

function submit() {
    form.put(route('calendar-types.update', props.calendarType.id));
}
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4 md:p-6 max-w-2xl mx-auto w-full">
            <div>
                <h1 class="text-2xl font-semibold tracking-tight text-foreground">Editar Tipo de Evento</h1>
                <p class="text-sm text-muted-foreground">Atualize as informações do tipo de evento ({{ calendarType.name }}).</p>
            </div>
