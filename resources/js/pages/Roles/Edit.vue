...
const form = useForm({
    ...
    permissions: props.role.permissions ? props.role.permissions.map((p: any) => p.name) : [] as string[],
});
...
function submit() {
    form.put(route('roles.update', props.role.id), {
        onSuccess: () => {
            // O success toast é lidado pelo layout/global baseado no flash
        },
        onError: () => {
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-semibold tracking-tight text-foreground">
                        Editar Grupo: {{ role.name }}
                    </h1>
                    <p class="text-sm text-muted-foreground">
                        Altere as configurações e os acessos deste grupo.
                    </p>
                </div>
