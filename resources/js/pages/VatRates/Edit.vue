...
const breadcrumbs = [
    ...
    { title: 'Editar Taxa', href: `/configuracoes/financeiro/iva/${props.vatRate.id}/editar` },
];

const form = useForm({
    name: props.vatRate.name,
    value: String(props.vatRate.value),
});
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4 md:p-6 max-w-2xl mx-auto w-full">
            <div>
                <h1 class="text-2xl font-semibold tracking-tight text-foreground">Editar Taxa de IVA</h1>
                <p class="text-sm text-muted-foreground">Atualize as informações da taxa de IVA ({{ vatRate.name }}).</p>
            </div>
